...
func TestKClosestPointstoOrigin(t *testing.T) {
	tests := []struct {
		...
		// 填入 test case
		{
			...
			utils.Sort2DIntArray([][]int{{-2, 2}}),
		},
		{
			parameters0973{[][]int{{3, 3}, {5, -1}, {-2, 4}}, 2},
			utils.Sort2DIntArray([][]int{{3, 3}, {-2, 4}}),
		},
		{
			parameters0973{[][]int{{0, 1}, {1, 0}}, 2},
			utils.Sort2DIntArray([][]int{{0, 1}, {1, 0}}),
		},
	}
	...
	for _, test := range tests {
		t.Run("Test KClosestPointstoOrigin", func(t *testing.T) {
			// 完整輸入參數
			result := utils.Sort2DIntArray(KClosestPointstoOrigin(test.parameters0973.points, test.parameters0973.k))
			// compare 的方式需視情況調整
